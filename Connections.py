...
def update_data():
    ...
    for doc in comment_data:
        ...
        for video in videos:
            video_id = video.get('Video_Id_1')  # Consider only the first video_id
            if video_id:
                comments = video.get('Comments', {})  # Get the 'Comments' field, default to an empty dict if not present
